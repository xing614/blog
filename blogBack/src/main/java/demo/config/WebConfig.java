...
//    	converters.add(0,mappingJackson2HttpMessageConverter());//我又把它注释了是因为与uedtior冲突，原因未找到

    	converters.add(mappingJackson2HttpMessageConverter());//发现不用这个会类型转换异常，不能给每个控制层方法转成json，用了这个之后ueditor没错，可能string类型赚不了
    }
